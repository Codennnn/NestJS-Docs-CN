...
function Toasts({
  position,
  viewportClassName,
}: { position: ToastPosition, viewportClassName?: string }) {
  ...
  return (
    <Toast.Portal container={contextContainer} data-slot="toast-portal">
      <Toast.Viewport
        className={cn(
...
function AnchoredToasts({
  ...
  return (
    <Toast.Portal container={contextContainer} data-slot="toast-portal-anchored">
      <Toast.Viewport
      ...
      >
        {toasts.map((toast) => {
          const Icon = toast.type
            ...
            : null
          const tooltipStyle
            = Boolean((toast.data as { tooltipStyle?: boolean }).tooltipStyle)
          ...
          return (
            <Toast.Positioner
              key={toast.id}
              className={cn(
                'z-100 max-w-[min(--spacing(64),var(--available-width))]',
                positionerClassName,
              )}
...
function AnchoredToastProvider({
  ...
  return (
    <Toast.Provider toastManager={anchoredToastManager} {...props}>
      {children}
      <AnchoredToasts positionerClassName={positionerClassName} />
    </Toast.Provider>
  )
}
