...
该过滤器提供了一种机制，可在可配置的最大次数内对 Kafka 事件进行重试处理。当发生异常时，它会将消息重新发布到同一主题，并在消息头中递增 `retry-count` 的值，然后提交当前偏移量。一旦达到最大重试次数，将触发自定义的 `skipHandler`（如果已提供）并提交偏移量，从而跳过该问题事件，确保后续事件的处理不受阻断。

你可以通过全局注册或在控制器层级注册来集成该过滤器。请注意，你需要提供一个 Kafka producer 实例：

```ts filename='kafka-retry.filter.ts'
...
export class AppKafkaRetryFilter extends KafkaMaxRetryExceptionFilter {
  constructor(@Inject('KAFKA_PRODUCER') producer: Producer) {
    super(producer, 5) // 最大重试次数 = 5
  }
}
```

```ts filename='my-event.handler.ts'
@Controller()
